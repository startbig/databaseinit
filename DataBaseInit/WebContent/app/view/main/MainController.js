...
Ext.define('app.view.main.MainController', {
    ...
    onMenuItemSelect: function(tree, record, index, eOpts) {
        if ((record.get('xtype'))) {
            var moduleId = record.get('xtype');
            var title = record.get('text');
            //1: 找到 contentPanel 这个组件
            //查找组件几种基本方法：
            //A：Ext.getCmp('ID')
    ...
    goHome: function(tree, record, index, eOpts) {
        var contentPanel = Ext.getCmp('content-panel');
        var module = Ext.widget('column-stacked-100');
        Ext.suspendLayouts(); //暂停布局
        contentPanel.removeAll(true);
        contentPanel.add(module);
        contentPanel.setTitle('主页');
        Ext.resumeLayouts(true); //恢复布局         
