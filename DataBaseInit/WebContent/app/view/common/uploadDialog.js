Ext.define('Genomics.view.common.uploadDialog',{
	...
	items : [{
		...
		region : 'north',
		autoScroll : true,
		html : "<div id='queueId' margin-left=20/>",
		height:250
	},{
		xtype: 'panel',
		region : 'center',
		...
		afterrender : function(){
			$(function() {
			    $("#file_upload_test").uploadify({
			        ...
			        uploader  : extPath+'/upload/findFileUpload.action',
			        auto: false, //不自动提交
			        queueID : 'queueId',
			        fileTypeDesc: '仅能上传word跟excel文件',     //文件类型描述  
			        fileTypeExts: '*.doc;*.xls;*.docx;*.xlsx;*.zip;',     //上传文件类型
			        ...
			        onSelectError: function(file, errorCode, errorMsg){ //file选择失败后触发  
			      	...
			      	onUploadSuccess: function(file, data, response){ //上传成功后触发  
				         if("sizeError" == data){  
				        	 ...
				        	 Ext.getCmp('showMsg').body.update('<div style="color:red;padding:10 10;">上传成功!</div>');
				        	 showMsg("上传成功!");
				         }else{
				        	 Ext.getCmp('showMsg').body.update('<div style="color:red;padding:10 10;">上传失败!错误原因如下:</div>');
				        	 showMsg("上传失败!");
				         }
			      	}
			    });
